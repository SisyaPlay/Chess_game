	...
	public void drawSquere(Graphics g) {
		...
		for (row = 0; row < ROW_COUNT; row++) {
			for (col = 0; col < ROW_COUNT; col++) {
				...
				y2 = y1 + square_size;
				if ((row % 2) == (col % 2)) {
					Cell cell = new Cell(x1 + shiftX, x1 + shiftY, x2 + shiftX, y2 + shiftY, square_size, Color.WHITE);
					cell.paintComponent(g2);
                } else {
                	Cell cell = new Cell(x1 + shiftX, x1 + shiftY, x2 + shiftX, y2 + shiftY, square_size, Color.black);
					cell.paintComponent(g2);
